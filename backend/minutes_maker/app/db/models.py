...
They mirror the tables created by the initial Alembic migration so that
`alembic revision --autogenerate` should yield **no diff**.
"""
...
# Every table in this module belongs to the "minutes" schema
...
class File(Base):
    ...
    # relationships
    transcripts: Mapped[List["Transcript"]] = relationship(back_populates="file", cascade="all, delete-orphan")
...
class Transcript(Base):
    ...
    versions: Mapped[List["MinutesVersion"]] = relationship(back_populates="transcript", cascade="all, delete-orphan")
    messages: Mapped[List["Message"]] = relationship(back_populates="transcript", cascade="all, delete-orphan")

    __table_args__ = (
        Index("transcripts_ts_idx", "ts", postgresql_using="gin"),
    )
...
class TranscriptChunk(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    transcript_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("minutes.transcripts.id", ondelete="CASCADE"))
    ...
    __table_args__ = (
        Index("transcript_chunks_embedding_ivfflat", "embedding", postgresql_using="ivfflat", postgresql_with={"lists": "100"}),
        Index("transcript_chunks_ts_idx", "ts", postgresql_using="gin"),
    )


# ---------------------------------------------------------------------------
# Table: minutes_versions
# ---------------------------------------------------------------------------
class MinutesVersion(Base):
    __tablename__ = "minutes_versions"

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    transcript_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("minutes.transcripts.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("transcript_id", "version_no", name="uix_minutes_versions_no"),
    )
...
class Message(Base):
    ...
    # relationships
    transcript: Mapped[Optional["Transcript"]] = relationship(back_populates="messages")
